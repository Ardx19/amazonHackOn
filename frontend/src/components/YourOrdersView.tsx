...
import { evaluateRoute } from '../lib/api';
...
export default function YourOrdersView({ orders, onAddToCart, onGoHome, onReturnReplace }: YourOrdersViewProps) {
  ...
  // ReRoute intercept modal
  const [rerouteResult, setRerouteResult] = useState<RoutingResult | null>(null);
  ...
  const handleReturnSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const routeResult = await evaluateRoute({
        item_id: firstItem?.id || selectedReturnOrder.id,
        original_price_inr: selectedReturnOrder.subtotal,
        category: firstItem?.category || 'electronics',
        current_location: {
          hub_id: 'MUM_H1',
          distance_to_home_warehouse_km: 48,
        },
        ring_index: 0,
      });

      if (routeResult.entered_reroute) {
        setRerouteResult(routeResult);
      ...
      setReturnSuccessMessage(`Return processed. ₹${refundAmount.toLocaleString('en-IN')} refunded to your Amazon Pay balance.`);
      setTimeout(() => setReturnSuccessMessage(null), 6000);
    } catch (err: any) {
      setIsSubmitProcess(false);
      setReturnSuccessMessage(`Error: ${err.message}`);
      setTimeout(() => setReturnSuccessMessage(null), 6000);
    }
  };

  const handleRerouteAccept = () => {
    if (!rerouteResult || !selectedReturnOrder) return;
    const refundAmount = returnActionType === 'refund' ? selectedReturnOrder.subtotal : 0;
    if (onReturnReplace) {
      onReturnReplace(selectedReturnOrder.id, 'Return Requested', refundAmount);
    }
    setReturnSuccessMessage(
      `ReRoute intercept accepted! ₹${rerouteResult.sale_price_inr.toFixed(0)} with ${rerouteResult.overhead_ratio ? (rerouteResult.overhead_ratio * 100).toFixed(1) : '—'}% overhead. Available within ${rerouteResult.profitable_radius_km.toFixed(1)}km.`
    );
  ...
  return (
    <div id="your-orders-tab-view" className="max-w-4xl mx-auto px-4 py-8 select-none">
      ...
      {/* ReRoute Intercept Modal — shows when entered_reroute === true */}
      {rerouteResult && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-white border-3 border-black p-6 max-w-md w-full shadow-[8px_8px_0px_#000] animate-in zoom-in-95 duration-200">
            <div className="flex items-center gap-2 mb-4">
              <span className="bg-emerald-500 text-white font-black text-xs px-2 py-1 border-2 border-black uppercase">ReRoute Intercept</span>
              <span className="text-amzn-orange font-bold text-sm">Amazon ReRoute Available</span>
            </div>

            <p className="text-xs text-gray-600 mb-4 leading-relaxed">
              Instead of a standard return, this item qualifies for the ReRoute cascade program.
              It will be listed at a floating discount for nearby buyers, saving logistics costs
              and giving you a faster refund.
            </p>

            <div className="grid grid-cols-2 gap-3 mb-4 bg-gray-50 p-3 border-2 border-black text-xs font-mono">
              <div>
                <span className="block text-[10px] text-gray-500 uppercase">Sale Price</span>
                <span className="text-lg font-black text-emerald-600">₹{rerouteResult.sale_price_inr.toFixed(0)}</span>
              </div>
              <div>
                <span className="block text-[10px] text-gray-500 uppercase">Discount</span>
                <span className="text-lg font-black text-red-600">{rerouteResult.discount_pct ? rerouteResult.discount_pct.toFixed(0) : '—'}% off</span>
              </div>
              <div>
                <span className="block text-[10px] text-gray-500 uppercase">Radius</span>
                <span className="text-sm font-bold">{rerouteResult.profitable_radius_km.toFixed(1)} km</span>
              </div>
              <div>
                <span className="block text-[10px] text-gray-500 uppercase">Ring</span>
                <span className="text-sm font-bold">Ring {rerouteResult.ring_index}</span>
              </div>
            </div>

            <div className="bg-emerald-50 border border-emerald-300 p-2.5 text-[11px] text-emerald-800 mb-4 rounded">
              <strong>ReRoute Reason:</strong> {rerouteResult.routing_reason}
            </div>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                type="button"
                onClick={handleRerouteAccept}
                className="px-5 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold rounded border border-emerald-600 shadow cursor-pointer"
              >
                Accept ReRoute
              </button>
            </div>
